...
const PredictionForm = () => {
  ...
  const highlightReason = (reason) => {
    const keywords = ["Loan-to-Income ratio", "CIBIL score", "Assets", "Annual income", "Strong financial profile"];
    let highlighted = reason;
    keywords.forEach((word) => {
      const regex = new RegExp(`(${word})`, "gi");
      highlighted = highlighted.replace(regex, `<span class="font-bold text-yellow-400">$1</span>`);
    });
    return highlighted;
  };

  // Save loan record to backend
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="dark:bg-gray-800 bg-white rounded-2xl shadow-md p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2">
          <button type="submit" disabled={loading} className={`w-full py-3 rounded-lg font-semibold text-white transition-colors ${loading ? "bg-gray-600 cursor-not-allowed" : "bg-indigo-600 hover:bg-indigo-700"}`}>
            {loading ? (
        ...
        {modelPrediction && (
          <div className="md:col-span-2 text-center text-lg mt-6 font-medium border-t border-gray-600 pt-4">
            <p>
              Model Prediction:{" "}
              <span className={modelPrediction === "Approved" ? "text-green-500" : "text-red-500"}>
                {modelPrediction}
              </span>
            </p>
            {modelReasons.length > 0 && (
              <ul className="mt-3 list-disc list-inside text-left md:text-center space-y-1">
                {modelReasons.map((r, i) => (
                  <li key={i} className="text-sm text-gray-300" dangerouslySetInnerHTML={{ __html: highlightReason(r) }} />
                ))}
              </ul>
            )}
          </div>
        ...
        {prediction && (
          <div className="md:col-span-2 text-center text-lg mt-6 font-medium border-t border-gray-600 pt-4">
            <div className="flex justify-center items-center space-x-2">
              <p>
                Final Decision:{" "}
                <span className={prediction === "Approved" ? "text-green-500" : "text-red-500"}>
                  {prediction}
                </span>
              </p>
